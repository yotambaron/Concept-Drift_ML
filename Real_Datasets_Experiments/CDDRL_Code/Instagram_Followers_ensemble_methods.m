...
save_path = 'D:\yotam\MATLAB\Stress_Experiments\CD_Real_Datasets\Results\Instagram_Followers\CDDRL\Ensemble_Methods';
...
for b=1:weeks
    ...
    [weights, AUCs_methods{b}] = update_ensemble_weights(weights, scores, true, weight_method, 1, classes);
    ...
    for b=1:weeks
        if b==1
            pred = preds_array(1:break_points(b), 1);
            true = df(1:break_points(b), n); 
        else
            pred = preds_array(break_points(b-1)+1:break_points(b), 1);
            true = df((break_points(b-1)+1):break_points(b), n);
