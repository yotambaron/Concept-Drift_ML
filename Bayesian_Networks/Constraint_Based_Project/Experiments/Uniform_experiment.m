...
df_path = 'C:\Users\User\Desktop\yotam\MATLAB\Constrain-Based CDDRL\DBs\Uniform';
save_path = 'C:\Users\User\Desktop\yotam\MATLAB\Constrain-Based CDDRL\Results\Uniform';
...
for p=1:3
    ...
    df = readtable([df_path '\\' 'Uniform_data' num2str(p) '.csv']);
    df = table2array(df);
    n_features = size(df, 2);    
    df(:, n_features) = df(:, n_features) + 1;
    ...
    if cddrl
        ...
        bnet.names = {'var_1', 'var_2', 'var_3', 'var_4', 'var_5', 'var_6', 'var_7', 'var_8', 'var_9', 'var_10', 'var_11', 'var_12', 'var_13', 'var_14', 'var_15', 'class'};
...
if cddrl
    ...
    if cb
        writetable(All_Results,[save_path '\try\Uniform_CDDRL_CB_Results.csv']);
    else
        writetable(All_Results,[save_path '\Uniform_CDDRL_S&S_Results.csv']);
    end
    ...
    writetable(All_Results,[save_path '\Uniform_Rest_Results.csv']);
    writetable(All_RunTimes,[save_path '\Uniform_Rest_RunTimes.csv'], 'WriteRowNames', 1);
end
